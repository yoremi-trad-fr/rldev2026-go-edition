...
func TestSetEncoding(t *testing.T) {
	...
	if GetMode() != EncWestern {
		t.Error("should be Western")
	}
...
func TestDescribe(t *testing.T) {
	SetMode(EncNone)
	if Describe() != "no output transformation" {
		t.Error("none")
	}
	SetMode(EncChinese)
	if Describe() == "no output transformation" {
...
func TestToBytecodeEmpty(t *testing.T) {
	...
	if len(b) != 0 {
		t.Error("empty should produce empty")
	}
}
...
func TestWesternKeepsNativeCP932(t *testing.T) {
	orig := text.Text{0x58f0} // 声
	b, err := encodeWestern(orig)
	if err != nil {
		t.Fatal(err)
	}
	want, err := encoding.UTF8ToSJS(string(rune(0x58f0)))
	if err != nil {
		t.Fatal(err)
	}
	if string(b) != string(want) {
		t.Fatalf("Western native CP932 bytes: got % x, want % x", b, want)
	}
	decoded, err := decodeWestern(b)
	if err != nil {
		t.Fatal(err)
	}
	if text.ToUTF8(decoded) != string(rune(0x58f0)) {
		t.Fatalf("Western native CP932 decode: got %q", text.ToUTF8(decoded))
	}
}
...
func TestWesternForceEncode(t *testing.T) {
	...
	b, err := encodeWestern(text.Text{0x1F600}) // 😀 (not in CP1252/CP932)
	if err != nil {
		t.Fatal(err)
	}
	if len(b) != 1 || b[0] != 0x20 {
		t.Errorf("forced: got %x", b)
	}
}

func TestWesternUnmappableError(t *testing.T) {
	ForceEncode = false
	_, err := encodeWestern(text.Text{0x1F600}) // 😀
	if err == nil {
		t.Error("should error for unmappable char without ForceEncode")
	}
}
...
func TestBadCharTracking(t *testing.T) {
	...
	if msg == "" {
		t.Error("should produce message")
	}
	if BadCharCount() != 1 {
		t.Errorf("count: %d", BadCharCount())
	}

	// Second complaint for same char
	Complain(0x1234, "test")
	if BadCharCount() != 1 {
		t.Error("same char should not increase distinct count")
	}
...
func TestReadBytecodeASCII(t *testing.T) {
	...
	if text.ToUTF8(txt) != "Hello" {
		t.Errorf("got %q", text.ToUTF8(txt))
	}
}
