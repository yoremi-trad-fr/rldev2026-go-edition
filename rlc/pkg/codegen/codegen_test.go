...
func TestChainCodes(t *testing.T) {
	if ChainCode(ast.ChainAnd) != 0x3c {
		t.Error("ChainAnd")
	}
	if ChainCode(ast.ChainOr) != 0x3d {
		t.Error("ChainOr")
	}
}
...
func TestEncodeInt32(t *testing.T) {
	...
	v = binary.LittleEndian.Uint32(b[2:])
	if int32(v) != -1 {
		t.Errorf("negative: got %d", int32(v))
	}
}
...
func TestEncodeOpcode(t *testing.T) {
	...
	if b[1] != 0 {
		t.Errorf("type: got %d", b[1])
	}
	if b[2] != 1 {
		t.Errorf("module: got %d", b[2])
	}
	code := binary.LittleEndian.Uint16(b[3:5])
	if code != 5 {
		t.Errorf("code: got %d", code)
	}
	argc := binary.LittleEndian.Uint16(b[5:7])
	if argc != 2 {
		t.Errorf("argc: got %d", argc)
	}
	if b[7] != 0 {
		t.Errorf("overload: got %d", b[7])
	}
}
...
func TestOutputAddCode(t *testing.T) {
	...
	if o.Length() != 2 {
		t.Errorf("length: got %d", o.Length())
	}
}
...
func TestOutputEmitExprInt(t *testing.T) {
	...
	if len(o.IR[0].Bytes) != 6 {
		t.Errorf("bytes: %d", len(o.IR[0].Bytes))
	}
}
...
func TestOutputEmitResRefQuotesUnsafeText(t *testing.T) {
	o := NewOutput()
	...
	if !bytes.Equal(got, []byte("\"MAY(04)\"")) {
		t.Fatalf("resource text with parens should be quoted, got %q", string(got))
	}
}
...
func TestOutputEmitResRefEmptyString(t *testing.T) {
	...
	if !bytes.Equal(got, []byte("\"\"")) {
		t.Fatalf("empty resource should emit an empty quoted string, got %q", string(got))
	}
}
...
func TestGenerateEmpty(t *testing.T) {
	...
	// Check magic. codegen emits raw bytecode and leaves compression to kprl -a.
	...
	if cv != 10002 {
		t.Errorf("compiler version: got %d", cv)
	}
}
