...
func TestChooseOverloadMultiple(t *testing.T) {
	...
	if idx != 0 {
		t.Errorf("argc=1: got %d, want 0", idx)
	}
	...
	if idx != 1 {
		t.Errorf("argc=2: got %d, want 1", idx)
	}
}
...
func TestChooseOverloadFallbackArb(t *testing.T) {
	...
	if idx != 1 {
		t.Errorf("got %d, want 1 (arbitrary)", idx)
	}
}
...
func TestAssembleBasic(t *testing.T) {
	...
	if len(result.Code) < 8 {
		t.Fatalf("code too short: %d", len(result.Code))
	}
	if result.Code[0] != '#' {
		t.Error("missing opcode prefix")
	}
	if result.Append != nil {
		t.Error("unexpected append")
	}
}
...
func TestAssembleOpcodeEncoding(t *testing.T) {
	...
	if result.Code[1] != 0 {
		t.Errorf("type: %d", result.Code[1])
	}
	if result.Code[2] != 3 {
		t.Errorf("module: %d", result.Code[2])
	}
...
func TestClassifyExpr(t *testing.T) {
	if ClassifyExpr(ast.IntLit{Val: 1}) != ETInt {
		t.Error("IntLit")
	}
	if ClassifyExpr(ast.StoreRef{}) != ETInt {
		t.Error("StoreRef")
	}
	if ClassifyExpr(ast.IntVar{Bank: 0, Index: ast.IntLit{}}) != ETInt {
		t.Error("IntVar")
	}
	if ClassifyExpr(ast.StrVar{Bank: 0, Index: ast.IntLit{}}) != ETStr {
		t.Error("StrVar")
	}
	...
	if ClassifyExpr(ast.ParenExpr{Expr: ast.IntLit{}}) != ETInt {
		t.Error("Parens(Int)")
	}
}

func TestCheckParamType(t *testing.T) {
	if CheckParamType(kfn.PAny, ETInt) != "" {
		t.Error("any/int")
	}
	if CheckParamType(kfn.PAny, ETStr) != "" {
		t.Error("any/str")
	}
	if CheckParamType(kfn.PIntC, ETInt) != "" {
		t.Error("intC/int")
	}
	if CheckParamType(kfn.PIntC, ETStr) == "" {
		t.Error("intC/str should fail")
	}
	if CheckParamType(kfn.PStrC, ETStr) != "" {
		t.Error("strC/str")
	}
	if CheckParamType(kfn.PStrC, ETLiteral) != "" {
		t.Error("strC/literal")
	}
	if CheckParamType(kfn.PStrC, ETInt) == "" {
		t.Error("strC/int should fail")
	}
	if CheckParamType(kfn.PStr, ETStr) != "" {
		t.Error("str/str")
	}
	if CheckParamType(kfn.PStr, ETInt) == "" {
		t.Error("str/int should fail")
	}
}
...
func TestLookupFuncDefDisambiguate(t *testing.T) {
	...
	if fd.OpCode != 1 {
		t.Errorf("int param: got opcode %d, want 1", fd.OpCode)
	}
	...
	if fd.OpCode != 2 {
		t.Errorf("str param: got opcode %d, want 2", fd.OpCode)
	}
}

func TestLookupFuncDefDisambiguateByArity(t *testing.T) {
	reg := kfn.NewRegistry()
	reg.Register(&kfn.FuncDef{Ident: "GetCursorPos", OpCode: 133,
		Prototypes: []kfn.Prototype{{Defined: true, Params: []kfn.Parameter{
			{Type: kfn.PInt}, {Type: kfn.PInt}, {Type: kfn.PInt}, {Type: kfn.PInt},
		}}}})
	reg.Register(&kfn.FuncDef{Ident: "GetCursorPos", OpCode: 202,
		Prototypes: []kfn.Prototype{{Defined: true, Params: []kfn.Parameter{
			{Type: kfn.PInt}, {Type: kfn.PInt},
		}}}})

	fd, err := LookupFuncDef(reg, "GetCursorPos", []ast.Param{
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 0}}},
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 1}}},
	}, false)
	if err != nil {
		t.Fatal(err)
	}
	if fd.OpCode != 202 {
		t.Errorf("2-arg call: got opcode %d, want 202", fd.OpCode)
	}

	fd, err = LookupFuncDef(reg, "GetCursorPos", []ast.Param{
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 0}}},
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 1}}},
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 2}}},
		ast.SimpleParam{Expr: ast.IntVar{Bank: 0, Index: ast.IntLit{Val: 3}}},
	}, false)
	if err != nil {
		t.Fatal(err)
	}
	if fd.OpCode != 133 {
		t.Errorf("4-arg call: got opcode %d, want 133", fd.OpCode)
	}
}
...
func TestBuildParamDefsExact(t *testing.T) {
	...
	if defs[0].Type != kfn.PIntC {
		t.Error("def 0")
	}
	if defs[1].Type != kfn.PStrC {
		t.Error("def 1")
	}
}
...
func TestBuildParamDefsArgcExtend(t *testing.T) {
	...
	for i := 1; i < 5; i++ {
		if defs[i].Type != kfn.PIntC {
			t.Errorf("def %d: got %v", i, defs[i].Type)
		}
	}
}
