...
func TestTableHasNamaeDefault(t *testing.T) {
	tbl := NewTable()
	if tbl.HasNamae() {
		t.Error("new table should not report #NAMAE")
	}
}
...
func TestParseSimple(t *testing.T) {
	...
	if v == nil {
		t.Fatal("WINDOW_ATTR not found")
	}
	if len(v) != 3 {
		t.Fatalf("values: %d", len(v))
	}
	if v[0].Int != 128 || v[2].Int != 190 {
		t.Errorf("values: %v", v)
	}
}
...
func TestParseString(t *testing.T) {
	...
	if v[0].Str != "VisualArt's" {
		t.Errorf("got %q", v[0].Str)
	}
}

func TestParseNamaeMarksTableWithoutCountingDefinition(t *testing.T) {
	tbl := parseStr(`#NAMAE = "声" = "Voice" = (0, -1, -1)`)
	if !tbl.HasNamae() {
		t.Fatal("expected #NAMAE marker")
	}
	if tbl.Count() != 0 {
		t.Fatalf("#NAMAE should remain metadata, count=%d", tbl.Count())
	}
}
...
func TestParseNoValue(t *testing.T) {
	...
	if !tbl.Exists("EMPTY_KEY") {
		t.Error("EMPTY_KEY should exist")
	}
	v := tbl.Find("EMPTY_KEY")
	if len(v) != 0 {
		t.Errorf("expected empty values, got %d", len(v))
	}
}
...
func TestParseNegative(t *testing.T) {
	...
	if v[0].Int != -100 {
		t.Errorf("v[0]: %d", v[0].Int)
	}
	if v[1].Int != -200 {
		t.Errorf("v[1]: %d", v[1].Int)
	}
}
...
func TestParseDotIdent(t *testing.T) {
	...
	if tbl.GetInt("WINDOW.SIZE", 0) != 640 {
		t.Error("WINDOW.SIZE")
	}
}
...
func TestParseDotIntDotInt(t *testing.T) {
	...
	if tbl.GetInt("SEL.000.001", 0) != 128 {
		t.Error("SEL.000.001")
	}
}
...
func TestParseDotIntDotIdent(t *testing.T) {
	...
	if v == nil || v[0].Str != "frame" {
		t.Error("WAKU.000.NAME")
	}
}
...
func TestParseDotIntDotIntDotIdent(t *testing.T) {
	...
	if v == nil || v[0].Str != "s_ped" {
		t.Error("WAKU.020.000.FILENAME")
	}
}
...
func TestParseDotIntDotIdentDotIdent(t *testing.T) {
	...
	if v == nil || v[0].Str != "btn" {
		t.Error("ITEM.000.BASE.FILENAME")
	}
}
...
func TestParseDotIntDotIdentDotIdentDotIdent(t *testing.T) {
	...
	if v == nil || v[0].Str != "slider" {
		t.Error("5-level key (Clannad)")
	}
}
...
func TestParseDotIdentDotIntDotIdent(t *testing.T) {
	...
	if v == nil || v[0].Str != "test" {
		t.Error("SECTION.ITEM.003.NAME")
	}
}
...
func TestParseDotIntDotIdentDotInt(t *testing.T) {
	...
	if tbl.GetInt("DATA.010.ENTRY.005", 0) != 42 {
		t.Error("DATA.010.ENTRY.005")
	}
}
...
func TestParseShake(t *testing.T) {
	...
	if v == nil {
		t.Fatal("SHAKE.001 not found")
	}
	if len(v) != 1 || v[0].Kind != VRange {
		t.Fatalf("expected range: %v", v)
	}
	if len(v[0].Ints) != 3 {
		t.Errorf("range ints: %d", len(v[0].Ints))
	}
	if v[0].Ints[0] != 100 || v[0].Ints[2] != 300 {
		t.Errorf("values: %v", v[0].Ints)
	}
}
...
`
	tbl := parseStr(src)
	if tbl.GetInt("SCREENSIZE_MOD", -1) != 0 {
		t.Error("SCREENSIZE_MOD")
	}
	if tbl.GetInt("FONT_SIZE", 0) != 26 {
		t.Error("FONT_SIZE")
	}
	v := tbl.Find("CAPTION")
	if v == nil || v[0].Str != "CLANNAD Side Stories" {
		t.Error("CAPTION")
	}

	// 5-level key
	v = tbl.Find("WAKU.000.000.EXBTN_000_BTN.000")
	if v == nil || v[0].Str != "waku_ex0" {
		t.Error("5-level key")
	}
}
...
`
	...
	for i, key := range keys {
		...
		if v[0].Str != expected[i] {
			t.Errorf("%s: got %q, want %q", key, v[0].Str, expected[i])
		}
	}
}
...
#D = N`)
	...
	c := tbl.Find("C")
	if c[0].Kind != VEnabled || !c[0].Bool {
		t.Error("C kind")
	}
	d := tbl.Find("D")
	if d[0].Kind != VEnabled || d[0].Bool {
		t.Error("D kind")
	}
}
